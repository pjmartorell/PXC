   ...
   throws IOException 
{
 ...
 restrictions on the requested file name */
  ...
  if(file.exists())
 	{
		if(request.esAutoritzat()) sendFile(outStream,file);
		else
		{
			outStream.write("HTTP/1.1 401 Unauthorized \r\n".getBytes());
	      outStream.write("WWW-Authenticate: Basic realm= WallyWorld \r\n".getBytes());
	      outStream.flush();
	      outStream.close();
		}
	}	
  ...
  public boolean esAutoritzat() 
    ...
    for (int i=0; i<lines.size(); i++) 
      ...
      if (auth[0].equals("Authorization:")) {
        ...
        if (userpass[0].equals(usr) && userpass[1].equals(pwd))
          return true;
     	}
   	}
    return false;
